...
void CBlurayIsoCache::Start()
{
  ...
  {
    logM(LOGDEBUG, "CBlurayIsoCache::Start - disable cache, invalid source length {}",
         m_sourceLength);
    return;
  }
  ...
  logM(LOGDEBUG,
       "CBlurayIsoCache::Start - cache config pageSize={} maxBytes={} maxPages={} forwardPages={}",
       m_config.pageSize, m_config.maxBytes, m_maxPages, m_config.forwardPrefetchPages);

  logM(LOGINFO, "CBlurayIsoCache::Start - Bluray ISO cache started for {} bytes source",
       m_sourceLength);
}
...
CBlurayIsoCache::PagePtr CBlurayIsoCache::LoadPage(int64_t pageIndex, bool prefetch)
{
  ...
  {
    logM(LOGERROR, "CBlurayIsoCache::LoadPage - callback returned {} bytes, expected at most {}",
...
void CBlurayIsoCache::LogStats(const char* reason)
{
  ...
  logM(LOGDEBUG,
       "CBlurayIsoCache::LogStats - {} detail: reads={} reqBlocks={} pageHits={} misses={} syncLoads={} pfLoads={} pfFails={} evicts={} fwdPfs={} pages={}",
  ...
  logM(LOGINFO,
       "CBlurayIsoCache::LogStats - {}: {} reads, {} pageHits / {} pageMisses, {} pfLoads ({} failed), {} evicts",
       reason, m_readRequests.load(), m_pageHits.load(), m_pageMisses.load(),
...
void CBlurayIsoCache::Worker()
{
  while (true)
  {
    ...
    {
      ++m_prefetchFailures;
      logComponentM(LOGDEBUG, LOGBLURAY, "CBlurayIsoCache::Worker - failed prefetch page {}",
                    pageIndex);
    }
  }
}
