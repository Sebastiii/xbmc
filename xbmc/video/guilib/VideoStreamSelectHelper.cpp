...
namespace
{
constexpr int STREAM_ID_DISABLE = -2;
constexpr int STREAM_ID_NONE = -1;
...
struct AudioStreamInfoExt : AudioStreamInfo
{
  AudioStreamInfoExt(int id, const AudioStreamInfo& info) : AudioStreamInfo(info)
  {
    ...
    if (!g_LangCodeExpander.Lookup(info.language, languageDesc))
      languageDesc = g_localizeStrings.Get(13205);
...
void CloseActiveStreamSelectDialogs(int keepId)
{
  auto& wm = CServiceBroker::GetGUI()->GetWindowManager();
  for (int id : {WINDOW_DIALOG_SELECT_VIDEO_STREAM,
                 WINDOW_DIALOG_SELECT_AUDIO_STREAM,
                 WINDOW_DIALOG_SELECT_SUBTITLE_STREAM})
  {
    if (id == keepId)
      continue;
    if (wm.IsWindowActive(id))
    {
      CGUIDialogSelect* other = wm.GetWindow<CGUIDialogSelect>(id);
      if (other)
        other->Close();
    }
  }
...
{
  CloseActiveStreamSelectDialogs(WINDOW_DIALOG_SELECT_VIDEO_STREAM);
  ...
  if (!dialog)
  {
    logM(LOGERROR, "Unable to get WINDOW_DIALOG_SELECT_VIDEO_STREAM dialog instance");
    return;
  }
  ...
  for (const VideoStreamInfoExt& info : streams)
  {
    ...
    fileItem->SetProperty("stream.hdrtype", CStreamDetails::HdrTypeToString(hdrType));
...
{
  CloseActiveStreamSelectDialogs(WINDOW_DIALOG_SELECT_AUDIO_STREAM);

  CGUIDialogSelect* dialog{CServiceBroker::GetGUI()->GetWindowManager().GetWindow<CGUIDialogSelect>(
      WINDOW_DIALOG_SELECT_AUDIO_STREAM)};
  if (!dialog)
  {
    logM(LOGERROR, "Unable to get WINDOW_DIALOG_SELECT_AUDIO_STREAM dialog instance");
    return;
  }
...
{
  CloseActiveStreamSelectDialogs(WINDOW_DIALOG_SELECT_SUBTITLE_STREAM);

  CGUIDialogSelect* dialog{CServiceBroker::GetGUI()->GetWindowManager().GetWindow<CGUIDialogSelect>(
      WINDOW_DIALOG_SELECT_SUBTITLE_STREAM)};
  if (!dialog)
  {
    logM(LOGERROR, "Unable to get WINDOW_DIALOG_SELECT_SUBTITLE_STREAM dialog instance");
    return;
  }
