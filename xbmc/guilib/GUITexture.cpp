...
void CGUITexture::Render(int32_t depthOffset, int32_t overrideDepth)
{
  ...
  if (m_width > 0.0f && m_height > 0.0f && m_frameWidth >= 2.0f * m_width &&
      m_frameHeight >= 2.0f * m_height)
  {
    const auto& minifiedTex = m_texture.m_textures[m_currentFrame];
    if (!minifiedTex->IsMipmapped() &&
        minifiedTex->GetScalingMethod() == TEXTURE_SCALING::LINEAR &&
        CServiceBroker::GetSettingsComponent()->GetAdvancedSettings()->m_guiMinifiedMipmapping)
      minifiedTex->SetMipmapping();
  }
